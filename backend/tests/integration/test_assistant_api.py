...
async def test_stream_is_saved_in_order_and_replayed_once(api: AsyncClient, tenant: Tenant, tender: Tender) -> None:
    created = await api.post("/api/v1/assistant/conversations", headers=tenant.headers, json={"tender_id": str(tender.id)})
    assert created.status_code == 201, created.text
    path = f"/api/v1/assistant/conversations/{created.json()['id']}"
    body = {"request_id": str(uuid4()), "text": "Show the timeline", "artifact": {"kind": "timeline"}}
...
async def test_other_tenant_cannot_read_delete_or_generate(api: AsyncClient, tenant: Tenant, tender: Tender) -> None:
    created = await api.post("/api/v1/assistant/conversations", headers=tenant.headers, json={"tender_id": str(tender.id)})
    path = f"/api/v1/assistant/conversations/{created.json()['id']}"
    ...
    try:
        ...
        assert (await api.post(path + "/turns", headers=other.headers, json={"request_id": str(uuid4()), "text": "Tell me their turnover"})).status_code == 404
...
async def test_voice_unavailable_is_explicit(api: AsyncClient, tenant: Tenant, tender: Tender) -> None:
    ...
    created = await api.post("/api/v1/assistant/conversations", headers=tenant.headers, json={"tender_id": str(tender.id)})
    response = await api.post(f"/api/v1/assistant/conversations/{created.json()['id']}/voice-ticket", headers=tenant.headers)
